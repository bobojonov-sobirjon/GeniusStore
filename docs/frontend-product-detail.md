...
### Описания (два разных поля в админке)

| Поле API | Админка | Назначение |
|----------|---------|------------|
| `description` | **Описание товара** (вкладка «Основная информация») | Общий текст для всей карточки |
| `variantDescription` | **Описание варианта** (у каждого цвета/памяти) | Меняется при выборе цвета/памяти |
| `selectedVariant.description` | То же, что `variantDescription` | Дубль для удобства |
| `colors[].memories[].description` | Описание конкретного варианта | Можно показать до выбора |

**Не путать:** `description` — это не описание варианта. Для текста под ценой при смене цвета используйте `variantDescription` или `selectedVariant.description`.
...
Все значения в `characteristicGroups` задаются **вручную** в админке. Поля `source` и `isVariant` в API остаются для совместимости (`source: "manual"`, `isVariant: false`).
